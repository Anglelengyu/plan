...
<template>
  <el-container style="height: 100%;text-align: left">
    <el-aside width="70%" style="height: 100%">
      <el-card :body-style="{ padding: '0px' }" v-for="(data,index) in 4">
        <div @mouseenter="pEnter(index)"
             @mouseleave="pLeave()">
          <div style="position: absolute;right: 25%;width: 10%">
          ...
          <div>
            <p style="overflow: hidden;text-overflow:ellipsis; white-space: nowrap;margin-left: 2%">
...
<script>
  ...
  export default {
    ...
    methods: {
      loadArticle() {
        this.loading = true;
        let url = "/" + this.currentPage + "/" + pageSize;
        this.getRequest('/login'+url).then(resp => {
          this.loading = false;
